 ...
 * @return string
 */
if (!function_exists('decode_emoji')) {
    function decode_emoji($text, $emojis)
    {
        if (!is_null($emojis)) {
 ...
 * @return string
 */
if (!function_exists('decode_stickers')) {
    function decode_stickers($text, $stickers)
    {
        if (!empty($stickers)) {
            foreach ($stickers as $sticker) {
                $replacement = '<img class="" src="' . url('/') . '/frontend/img/' . $sticker['image'] . '"></i>';
                $text = preg_replace('/(^|\s):STK-' . $sticker['id'] . ':/', $replacement, $text);
            }
        }
        return $text;
    }
